...
export default function LearnScreen({ navigation, route }) {
  ...
  const handleSubmit = async () => {
    if (!answers[currentQuestion]) { Alert.alert('Please provide an answer'); return; }
    setIsSubmitting(true);
    try {
      const userId = user?.id || 'user';
      const paddedAnswers = questions.map((_, i) => i === currentQuestion ? answers[currentQuestion] : '');
      let response;

      if (activity.type === 'listening') {
        if (activity.difficulty === 0) {
          const raw = await evaluateAPI.evaluateListeningEasy(activity._id || activity.id, paddedAnswers, userId);
          response = { ...raw, results: raw.results ? [raw.results[currentQuestion]] : raw.results };
        } else if (activity.difficulty === 1) {
          response = await evaluateAPI.evaluateListeningMedium(activity._id || activity.id, answers[currentQuestion], userId);
        } else {
          const raw = await evaluateAPI.evaluateListeningHard(activity._id || activity.id, paddedAnswers, userId);
          response = { ...raw, results: raw.results ? [raw.results[currentQuestion]] : raw.results };
        }
      } else if (activity.type === 'writing') {
        if (activity.difficulty === 0) {
          const raw = await evaluateAPI.evaluateWritingEasy(activity._id || activity.id, paddedAnswers, userId);
          response = { ...raw, results: raw.results ? [raw.results[currentQuestion]] : raw.results };
        } else if (activity.difficulty === 1) {
          response = await evaluateAPI.evaluateWritingMedium(activity._id || activity.id, answers[currentQuestion], userId);
        } else if (activity.difficulty === 2) {
          const raw = await evaluateAPI.evaluateWritingHard(activity._id || activity.id, answers[currentQuestion], userId);
          response = { ...raw, results: raw.results ? [raw.results[currentQuestion]] : raw.results };
        }
      } else if (activity.type === 'reading') {
        if (activity.difficulty === 0) {
          const raw = await evaluateAPI.evaluateReadingEasy(activity._id || activity.id, paddedAnswers, userId);
          response = { ...raw, results: raw.results ? [raw.results[currentQuestion]] : raw.results };
        } else if (activity.difficulty === 1) {
          const raw = await evaluateAPI.evaluateReadingMedium(activity._id || activity.id, paddedAnswers, userId);
          response = { ...raw, results: raw.results ? [raw.results[currentQuestion]] : raw.results };
        } else {
          const raw = await evaluateAPI.evaluateReadingHard(activity._id || activity.id, paddedAnswers, userId);
          response = { ...raw, results: raw.results ? [raw.results[currentQuestion]] : raw.results };
        }
      }

      setFeedback(response);
      setAllFeedback(prev => [...prev, { question: currentQuestion, ...response }]);
    } catch (error) {
  ...
  const handleFinish = async () => {
    // Refresh progress and then show the results screen
    if (selectedLanguage) {
